namespace Monads
{
    public readonly struct Carry<T, TCarry> : IEquatable<Carry<T, TCarry>>
    {
        ...
        {
            Value = value;
            Carried = carried.GetEmptyIfDefault();
        }
        ...
        public static Carry<T, TCarry> Unit(T value, TCarry carry) => new(value, carry);
        public static Carry<T, TCarry> Unit(T value, IEnumerable<TCarry> carried) => new(value, carried);
        public static Carry<T, TCarry> Unit(T value) => new(value);

        public Carry<TResult, TCarry> Bind<TResult>(Func<T, Carry<TResult, TCarry>> function) => function.Invoke(Value).Concat(Carried);
        public Carry<T, TCarry> Concat(IEnumerable<TCarry> carried) => Unit(Value, carried.GetEmptyIfDefault().Concat(Carried.GetEmptyIfDefault()));
